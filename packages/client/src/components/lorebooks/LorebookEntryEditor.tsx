// ──────────────────────────────────────────────
// Lorebook Entry Editor — extracted form
// ──────────────────────────────────────────────
// The full entry editor used in two places:
//   1. The route-level LorebookEditor (default headered layout, page mode)
//   2. The Active World Info quick-edit modal (headerless, host provides
//      Save/Cancel in the modal footer)
//
// All previously-internal helpers (FieldGroup, KeysEditor, ToggleButton,
// NumberField, ExpandableTextarea, ExpandedContentModal) live here too,
// since LorebookEditor no longer uses them outside this form.
import {
  useEffect,
  useRef,
  useState,
...
// ── Reusable sub-components (formerly internal to LorebookEditor) ────────
...
export interface LorebookEntryEditorProps {
  entryForm: Partial<LorebookEntry>;
  onChange: (patch: Partial<LorebookEntry>) => void;
  /** Save action. When `headerless` is true, the host (modal) likely
   *  invokes this from its own footer; the built-in header's "Save Entry"
   *  button is hidden. */
  onSave: () => void;
  saving: boolean;
  /** Optional back/exit. Shown as the ArrowLeft button in the built-in
   *  header. Not relevant in modal context. */
  onExit?: () => void;
  /** Hide the built-in header (back arrow + name input + Save button).
   *  The modal use-case sets this to true so its own title bar / footer
   *  controls aren't duplicated. The form body still renders normally. */
  headerless?: boolean;
}

/**
 * Full lorebook-entry editor form. Pure controlled component — no
 * mutations or fetches inside. Caller owns the entryForm state and
 * persistence.
 */
export function LorebookEntryEditor({ entryForm, onChange, onSave, saving, onExit, headerless }: LorebookEntryEditorProps) {
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      {!headerless && (
        <div className="flex items-center gap-3 border-b border-[var(--border)] px-4 py-3">
          {onExit && (
            <button onClick={onExit} className="rounded-lg p-1.5 transition-colors hover:bg-[var(--accent)]">
              <ArrowLeft size="1rem" />
            </button>
          )}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          >
            <KeysEditor keys={entryForm.keys ?? []} onChange={(keys) => onChange({ keys })} />
          </FieldGroup>
          ...
          >
            <KeysEditor
              ...
              onChange={(keys) => onChange({ secondaryKeys: keys })}
            />
            <div className="mt-2 flex items-center gap-3">
              ...
              {(["and", "or", "not"] as const).map((logic) => (
                <button
                  key={logic}
                  onClick={() => onChange({ selectiveLogic: logic })}
                  className={cn(
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <ToggleButton
              ...
              onChange={(v) => onChange({ useRegex: v })}
            />
            <ToggleButton
              label="Whole Words"
              value={entryForm.matchWholeWords ?? false}
              onChange={(v) => onChange({ matchWholeWords: v })}
            />
            <ToggleButton
            ...
            <ToggleButton
              ...
              onChange={(v) => onChange({ preventRecursion: v })}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => onChange({ role: e.target.value as "system" | "user" | "assistant" })}
                  className="w-full rounded-lg bg-[var(--secondary)] px-2 py-1.5 text-xs ring-1 ring-[var(--border)] focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
                >
          ...
          >
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <NumberField
                ...
                onChange={(v) => onChange({ sticky: v || null })}
                min={0}
              />
              <NumberField
                label="Cooldown"
                value={entryForm.cooldown ?? 0}
                onChange={(v) => onChange({ cooldown: v || null })}
                min={0}
              />
              <NumberField
                label="Delay"
                value={entryForm.delay ?? 0}
                onChange={(v) => onChange({ delay: v || null })}
                min={0}
              />
              <NumberField
                label="Ephemeral"
                value={entryForm.ephemeral ?? 0}
                onChange={(v) => onChange({ ephemeral: v || null })}
                min={0}
              />
            </div>
          </FieldGroup>
